 ...
 *
 */
Node.prototype.display = function(time) {
	...
	this.scene.multMatrix(this.getMatrix(time));
	...
	for(var i = 0; i < this.childrensComponent.length; i++){
		...
		this.scene.graph.components[this.childrensComponent[i]].display(time);
...
Node.prototype.getMatrix = function(time) {
	...
	if(time > this.tempoTotal) {
		
		
		var animation = this.scene.graph.animations[this.animations[this.animations.length - 1]];
		mat4.multiply(result, animation.getMatrix(animation.time), this.matrix);
	...
	for(var i = 0; i < this.animations.length; i++) {
		...
		var animation = this.scene.graph.animations[this.animations[i]];
		
		if(remainingTime < animation.time ){
			var animationMatrix = animation.getMatrix(remainingTime)
		 
		 
			mat4.multiply(result, animationMatrix, this.matrix );
			
			return result;
			
		}
		remainingTime -= animation.time;
...
Node.prototype.incializaAnimacoes = function() {
	this.tempoTotal = 0;
	for(var i = 0; i < this.animations.length; i++)
		this.tempoTotal += this.scene.graph.animations[this.animations[i]].time; 
	
};

Node.prototype.pushAnimation = function(animation) {
	this.animations.push(animation);
	this.incializaAnimacoes();
}
